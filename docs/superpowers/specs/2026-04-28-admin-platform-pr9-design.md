...
- **PR 12a forward-evolution: `punishedUserAccountId` 필드 추가** (commit `0a6519b3`): `AdminCrewPenalizedEvent`에 `punishedUserAccountId` 필드 추가. `UserActivityLogListener`(PR 12a)가 `user_account_id` 기준으로 `user_activity_log` row를 만들 때 cross-BC `crewId → user_account_id` lookup race를 회피하기 위해 이벤트 self-contain. `AdminCrewPenaltyCommandService.apply` publisher가 `crew.getUserId().getUid()`를 채워 publish. PR 9 단위/IT 테스트 fixture는 PR 12a G3에서 동시 갱신.
