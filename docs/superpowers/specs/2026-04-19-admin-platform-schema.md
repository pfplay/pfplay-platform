...
> V4~V13 Flyway 마이그레이션의 DDL, 전환 전략, 리팩토링 범위를 확정한다.
> 본 문서가 다루지 않는 슬롯: V11은 PR 6 `must_change_password` (참조: `admin-platform-security.md`).
...
| V11 | IAM | `user_account.must_change_password` 컬럼 (PR 6) — 자세한 설계는 `admin-platform-security.md` §5.6 | ALTER | 낮음 |
...
| V13 | Administration | `partyroom_report` 테이블 (PR 13) | CREATE | 낮음 |
...
## 4.8 V13 — partyroom_report

### 4.8.1 DDL (V13__create_partyroom_report.sql)
...
V10 (user_activity_log — user_account 참조, Administration)
  ↓
V11 (must_change_password — user_account 컬럼 추가, PR 6)
  ↓
V12 (Avatar BC 재구성 — 기존 V3 시드 위에 진행, 다른 V와 독립)
  ↓
V13 (partyroom_report — administrator FK 있음, PR 13)
```
...
> 원안 슬롯에 그대로 안착(V12). PR 13 시점에 Flyway slot renumber 결정으로 PR 6 `must_change_password`를 V11, Avatar BC를 V12로 재정렬하고 `out-of-order` 정책을 false로 전환했다. 자세한 history는 `2026-04-28-admin-platform-pr13-design.md` 참고.
...
- `lifecycle_status DEFAULT 'PUBLISHED'`는 V12 이후 역할이 끝남 (신규 INSERT는 aggregate가 `DRAFT` 명시). **REVISIT-LATER**: V12가 안정 후 후속 마이그레이션(V15+)에서 `ALTER TABLE ... ALTER COLUMN lifecycle_status DROP DEFAULT`로 제거하면 aggregate 우회 INSERT 실수를 조기 차단 가능. 낮은 우선순위.
