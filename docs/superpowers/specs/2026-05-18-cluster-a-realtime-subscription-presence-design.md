...
- `ConnectionEventListener`(현 passive): `SessionConnectEvent`에서 Principal=uid+sessionId 레지스트리 등록. 유저 pending이면 `clearPending`(**CONNECT 시점 — SE4 fix, SUBSCRIBE 비의존**).
  ...
  - **모듈 경계 seam(구현 발견, Option 3 채택)**: `realtime`는 `app`/`common` 미의존(단방향 `app→realtime`)이라 리스너가 `UserSessionRegistry`(app)·`UserId`(common) 직접 사용 불가, raw uid `String`만 보유. → 신규 최소 realtime 포트 `SessionRegistryPort.register(String sessionId, String userId)` + app측 `SessionRegistryPortAdapter`(→`UserSessionRegistry.register(sid, UserId.fromString(uid))`). 등록이 `onSessionConnected`(uid resolve) 선행 필요 → register는 app측 수행. `PresencePort`/`PresencePortAdapter`/`ClusterAPresenceIntegrationTest` 계약 무변경. unregister는 신규 포트 불요(기존 `PresencePortAdapter.onSessionDisconnected`가 app측 `UserSessionRegistry.unregister` 직접 수행).
...
- `SubscriptionEventListener`: **presence 트리거(`presencePort.onSessionConnected`)만 제거** — presence는 CONNECT 책임. **⚠️ 세션캐시(`saveSessionCache`/`deleteSessionCache`)는 유지**: `PartyroomChatCommandService.sendMessage`가 `getSessionCache(sessionId)`로 세션→(partyroom,user,crew)를 resolve(없으면 SESSION NotFound)하는 **비-presence 소비자**. 세션캐시를 presence 권위에서만 분리하고 chat용 write/delete 라이프사이클은 그대로 둔다(삭제 시 채팅 전면 파손 — 구현 중 발견된 cross-cutting 결합). **서버 단일룸 가드** 추가: SUBSCRIBE `/sub/partyrooms/{id}`의 id가 유저 권위 active 룸과 불일치 → 등록 거부(no-op, 로그). presence용 세션캐시 기록 제거.
