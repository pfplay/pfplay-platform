...
public class SelfUpdateConfig {
    ...
    public boolean isEnabled() {
        return cache.readBoolean(ConfigKey.VCREW_PLAYLIST_SELF_UPDATE_ENABLED, DEFAULT_ENABLED);
    }

    public int cooldownSeconds() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_COOLDOWN_SECONDS, DEFAULT_COOLDOWN_SECONDS);
    }

    public int minReactions() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_MIN_REACTIONS, DEFAULT_MIN_REACTIONS);
    }

    public int replacePerCycle() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_REPLACE_PER_CYCLE, DEFAULT_REPLACE_PER_CYCLE);
    }

    public int recommendCount() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_RECOMMEND_COUNT, DEFAULT_RECOMMEND_COUNT);
    }

    public int prunedCooldownSeconds() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_PRUNED_COOLDOWN_SECONDS, DEFAULT_PRUNED_COOLDOWN_SECONDS);
    }

    public double weightReaction() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_WEIGHT_REACTION, DEFAULT_WEIGHT_REACTION_PERMILLE) / 1000.0;
    }

    public double weightGrab() {
        return cache.readInt(ConfigKey.VCREW_SELF_UPDATE_WEIGHT_GRAB, DEFAULT_WEIGHT_GRAB_PERMILLE) / 1000.0;
    }
}
