...
public class VirtualCrewReconcileScheduler {
    ...
    private final VirtualCrewReactionConfig reactionConfig;
    ...
    public void reconcileManagedRooms() {
        if (maintenanceGate.isUnderMaintenance()) {
            log.debug("[vcrew-cron] SKIP_UNDER_MAINTENANCE");
            return;
        }
        ...
        if (selfUpdateConfig.isEnabled()) {
            for (PartyroomVirtualCrewConfigData cfg : managed) {
                try {
                    ...
                    log.warn("[vcrew-cron] self-update failed for partyroomId={} : {}",
        ...
        if (reactionConfig.isEnabled()) {
            for (PartyroomVirtualCrewConfigData cfg : managed) {
                try {
                    ...
                    log.warn("[vcrew-cron] reaction failed for partyroomId={} : {}",
                            cfg.getPartyroomId(), e.getMessage());
                }
            }
        }
