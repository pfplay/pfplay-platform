package com.pfplaybackend.api.virtualcrew.application.config;
 ...
 */
@Configuration
public class VirtualCrewChatAsyncConfig {

    /** {@code LlmChatTaskRunner} 가 {@code @Qualifier} 로 주입하는 빈 이름 — 컴파일 타임 linkage. */
    public static final String VCREW_CHAT_EXECUTOR = "vcrewChatExecutor";

    @Bean(name = VCREW_CHAT_EXECUTOR)
    public ThreadPoolTaskExecutor vcrewChatExecutor() {
        ThreadPoolTaskExecutor exec = new ThreadPoolTaskExecutor();
        exec.setCorePoolSize(2);
        exec.setMaxPoolSize(4);
        exec.setQueueCapacity(50);
        exec.setThreadNamePrefix("vcrew-chat-");
