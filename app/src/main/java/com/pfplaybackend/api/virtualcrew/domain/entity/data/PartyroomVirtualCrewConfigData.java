...
@DynamicInsert
@DynamicUpdate
@Table(name = "partyroom_virtual_crew_config")
@Entity
public class PartyroomVirtualCrewConfigData extends BaseEntity {
    ...
    private VirtualCrewStatus status;
    ...
    protected PartyroomVirtualCrewConfigData() {
    }

    @Builder
    public PartyroomVirtualCrewConfigData(Long partyroomId, VirtualCrewStatus status,
     ...
     */
    public static PartyroomVirtualCrewConfigData create(Long partyroomId) {
        return PartyroomVirtualCrewConfigData.builder()
                .partyroomId(partyroomId)
                .status(VirtualCrewStatus.OFF)
                .targetCount(2)
                .djBotCount(1)
                .build();
    }
    ...
    public void applyManaged(int targetCount, int djBotCount, Long songPackId) {
        this.status = VirtualCrewStatus.MANAGED;
        this.targetCount = targetCount;
        this.djBotCount = djBotCount;
        this.songPackId = songPackId;
    }
     ...
     */
    public void turnOff() {
        this.status = VirtualCrewStatus.OFF;
    }
