 ...
 * 봇 플레이리스트에 복사한다.
 *
 * <p>분배 규칙: {@code trackCount}를 {@code djCount} 조각으로 나눈다. 각 조각 크기는
 * {@code base = trackCount / djCount} 또는 {@code base + 1}(앞쪽 {@code remainder}개 조각).
 * 예) 10트랙 3조각 → [4, 3, 3].
 */
public final class TrackDistribution {
     ...
     *
     * @param djCount            설정된 크루(DJ) 봇 수
     * @param filteredTrackCount 룸 시간제한 통과 트랙 수
     * @return {@code min(djCount, filteredTrackCount)}
     */
     ...
     *
     * <p>{@code slotIndex} 는 {@code [0, djCount)} 범위여야 한다(호출측 보장). 그 범위 밖은
     * 빈 조각이 되므로 배정하지 않는다.
     *
