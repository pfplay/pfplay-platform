...
import com.pfplaybackend.api.virtualdj.adapter.in.event.VirtualDjMaintenanceListener;
...
@DisplayName("VirtualDjMaintenanceListener — 점검 이벤트 → sweeper 위임")
class VirtualDjMaintenanceListenerTest {

    private VirtualDjManagedRoomSweeper sweeper;
    private VirtualDjMaintenanceListener listener;

    @BeforeEach
    void setUp() {
        sweeper = mock(VirtualDjManagedRoomSweeper.class);
        listener = new VirtualDjMaintenanceListener(sweeper);
    }
