...
import com.pfplaybackend.api.virtualcrew.application.service.PlaylistSelfUpdateService;
 ...
 *
 * <p>봇을 <b>활성 봇 DJ</b> 로 만드는 셋업은 {@link VirtualCrewOrchestratorIT} 와 동일하게
...
class PlaylistSelfUpdateIT extends AbstractIntegrationTest {
    ...
    @Autowired private VirtualCrewOrchestrator orchestrator;
    ...
    private Fixture seedActiveBotDjRoom() {
        ...
        PartyroomVirtualCrewConfigData cfg = reloadConfig(roomId);
    ...
    private long seedRoom(int playbackTimeLimitMinutes) {
        PartyroomData p = PartyroomData.create(
                "vcrew-selfupdate", "intro", LinkDomain.of("link-vcrew-su-" + System.nanoTime()),
    ...
    private void seedManagedConfig(long roomId, int target, int floor, Long songPackId) {
        configRepository.save(PartyroomVirtualCrewConfigData.builder()
                .partyroomId(roomId)
                .status(com.pfplaybackend.api.virtualcrew.domain.enums.VirtualCrewStatus.MANAGED)
    ...
    private PartyroomVirtualCrewConfigData reloadConfig(long roomId) {
