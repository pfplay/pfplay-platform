...
import com.pfplaybackend.api.virtualcrew.application.service.BotSlotAssigner;
...
class BotPlacementServiceTest {
    ...
    private void stubConfig(int targetCount, int djBotCount, Long songPackId) {
        PartyroomVirtualCrewConfigData cfg = PartyroomVirtualCrewConfigData.builder()
                .partyroomId(ROOM.getId())
                .status(VirtualCrewStatus.MANAGED)
    ...
    void skipWhenNotManaged() {
        PartyroomVirtualCrewConfigData off = PartyroomVirtualCrewConfigData.builder()
                .partyroomId(ROOM.getId())
