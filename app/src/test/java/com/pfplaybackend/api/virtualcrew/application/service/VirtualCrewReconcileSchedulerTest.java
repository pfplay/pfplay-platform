...
/**
 * {@link VirtualCrewReconcileScheduler} 단위 테스트.
 *
 * <p>협력자 모두 mock — 점검 게이트 / self-update 루프 / 반응 루프 로직만 검증한다.
 */
class VirtualCrewReconcileSchedulerTest {
    ...
    private VirtualCrewReactionConfig reactionConfig;
    private BotReactionService botReactionService;
    private MaintenanceGate maintenanceGate;

    private VirtualCrewReconcileScheduler scheduler;
    ...
    void setUp() {
        ...
        reactionConfig = mock(VirtualCrewReactionConfig.class);
        ...
        scheduler = new VirtualCrewReconcileScheduler(
    ...
    private PartyroomVirtualCrewConfigData mockCfg(long partyroomId) {
    ...
    void disabled_selfUpdateNeverCalled() {
        // given
        PartyroomVirtualCrewConfigData cfg1 = mockCfg(1L);
        PartyroomVirtualCrewConfigData cfg2 = mockCfg(2L);
        when(configRepository.findByStatus(VirtualCrewStatus.MANAGED)).thenReturn(List.of(cfg1, cfg2));
    ...
    void enabled_tryUpdateRoomCalledPerRoom() {
        // given
        PartyroomVirtualCrewConfigData cfg1 = mockCfg(10L);
        PartyroomVirtualCrewConfigData cfg2 = mockCfg(20L);
        when(configRepository.findByStatus(VirtualCrewStatus.MANAGED)).thenReturn(List.of(cfg1, cfg2));
    ...
    void selfUpdateExceptionIsolated() {
        // given
        PartyroomVirtualCrewConfigData cfg1 = mockCfg(100L);
        PartyroomVirtualCrewConfigData cfg2 = mockCfg(200L);
        when(configRepository.findByStatus(VirtualCrewStatus.MANAGED)).thenReturn(List.of(cfg1, cfg2));
        when(selfUpdateConfig.isEnabled()).thenReturn(true);
    ...
    void emptyManagedList_earlyReturn() {
        // given
        when(configRepository.findByStatus(VirtualCrewStatus.MANAGED)).thenReturn(List.of());
    ...
    void reactionEnabled_tryReactCalledPerRoom() {
        // given
        PartyroomVirtualCrewConfigData cfg1 = mockCfg(101L);
        PartyroomVirtualCrewConfigData cfg2 = mockCfg(102L);
        when(configRepository.findByStatus(VirtualCrewStatus.MANAGED)).thenReturn(List.of(cfg1, cfg2));
        when(selfUpdateConfig.isEnabled()).thenReturn(false);
