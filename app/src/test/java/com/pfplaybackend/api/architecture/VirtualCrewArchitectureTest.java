 ...
 * </ul>
 *
 * <h2>규칙 B — {@code ..virtualcrew..} 패키지 전체 가드</h2>
 * virtualcrew 패키지의 어떤 클래스도 다음에 직접 의존해서는 안 된다:
 * <ul>
 ...
 * </ul>
 *
 * <p>오케스트레이터는 봇 신원({@link com.pfplaybackend.api.virtualcrew.application.identity.BotIdentityExecutor})
 ...
 * feature 의 config repo({@code PartyroomVirtualCrewConfigRepository}, 단일 row 설정 읽기 — 허용).
 */
@DisplayName("가상 DJ 아키텍처 의존성 가드 (Chunk 6 ArchUnit)")
class VirtualCrewArchitectureTest {
    ...
    @Test
    @DisplayName("virtualcrew 패키지 전체: admin BC 를 참조하지 않는다 (봇 계정 프로비저닝 어댑터만 예외)")
    void virtualCrewPackageMustNotDependOnAdmin() {
        // admin 역참조 금지를 virtualcrew 패키지 전체로 확대(고정 2역할 모델: 봇 로직은 admin BC 를
        // 알 필요가 없다). 유일한 승인 예외 = adapter/out/provision — 봇 더미 계정 생성은 본질적으로
        // 계정 관리(admin) 연산이라 AdminUserService 를 경유한다(이 리팩토링 이전부터의 정당한 의존).
        // 이 경계 밖의 어떤 virtualcrew 클래스도 admin 을 import 하면 안 된다.
        ArchRule rule = noClasses()
                .that().resideInAPackage("..virtualcrew..")
                .and().resideOutsideOfPackage("..virtualcrew.adapter.out.provision..")
    ...
    void orchestratorMustNotDependOnAdministration() {
        ...
        // (VirtualCrewMaintenanceListener 가 MaintenanceStartedEvent 등을 구독). orchestrator 만 금지.
        ArchRule rule = noClasses()
                .that().resideInAPackage("..virtualcrew..")
    ...
    void orchestratorMustNotDependOnDomainPersistence() {
        // PartyroomVirtualCrewConfigRepository(자기 feature 의 단일 row 설정 읽기)는 허용 —
        // 이름에 'VirtualCrewConfig' 가 들어가므로 매처에서 제외한다.
        ArchRule rule = noClasses()
                .that().resideInAPackage("..virtualcrew..")
                .and().haveSimpleNameContaining("Orchestrator")
                .should().dependOnClassesThat()
                .haveNameMatching(".*(AggregatePort|(?<!VirtualCrewConfig)Repository)$");
        rule.check(allClasses);
    }
    ...
    // virtualcrew 에서 publisher(RedisMessagePublisher) 를 직접 호출해서는 안 된다. 아래 규칙 B 가
    // 그 우회를 패키지 전체 범위에서 차단한다.

    /**
     * {@code ..virtualcrew..} 의 어떤 클래스도 {@code *MessagePublisher} 를 직접 의존하지 않는다.
     *
     ...
     *
     * <p><b>예외 없음:</b> 현재 virtualcrew 헬퍼({@code VirtualSongPackService}, {@code SongPackApplier},
     * {@code VirtualUserPoolService}, {@code ActiveDjSnapshotService}, {@code VirtualMemberProvisionAdapter},
     * {@code *RepositoryImpl} 등)는 모두 자체 virtualcrew repo + playlist/track/user repo 만 사용한다.
     */
    @Test
    @DisplayName("virtualcrew 패키지 전체: *MessagePublisher 를 직접 의존하지 않는다")
    void virtualCrewPackageMustNotDependOnMessagePublisher() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..virtualcrew..")
                .should().dependOnClassesThat()
    ...
    /**
     * {@code ..virtualcrew..} 의 어떤 클래스도 party {@code *AggregatePort} 를 직접 의존하지 않는다.
     *
     ...
     *
     * <p><b>예외 없음:</b> virtualcrew 는 party BC 와 상호작용할 때 반드시 application service 를
     * 경유해야 한다. 봇은 {@code BotIdentityExecutor} 로 실유저와 동일한 명령 경로를 탄다.
     */
    @Test
    @DisplayName("virtualcrew 패키지 전체: party *AggregatePort 를 직접 의존하지 않는다")
    void virtualCrewPackageMustNotDependOnPartyAggregatePort() {
        ArchRule rule = noClasses()
                .that().resideInAPackage("..virtualcrew..")
                .should().dependOnClassesThat()
